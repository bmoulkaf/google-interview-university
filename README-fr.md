...
Je suis ce plan pour préparer mon entretien à Google. Je construisais le web, bâtissais des services, et lançais des startups depuis 1997. J'ai un diplôme en économie, pas en informatique. J'ai eu beaucoup de succès dans ma carrière, mais je veux travailler à Google. Je veux progresser vers des systèmes plus grands et obtenir une réelle compréhension des systèmes informatiques, efficacité algorithmique, performance des structures de données, langages de bas niveau, et comment tout ça fonctionne. Et si vous n'en savez rien, Google ne vous recrutera pas. 

Quand j'ai commencé ce projet, je ne faisais aucune différence entre une pile et un tas, je ne savais absolument rien, rien sur les arbres, ou comment traverser un graph. Si j'avais eu à coder un algorithme de tri, je peux vous dire que ca n'aurait pas été très bien. Chaque structure de données que j'ai utilisée était intégré dans le langage, et je n'avais absolument aucune idée sur comment ça marchait.Je n'ai jamais eu à gérer la mémoire jusqu'à ce que le processus donnait l'erreur "mémoire insuffisante", et à ce moment-là j'avais à trouver une solution pour la contourner. J'ai utilisé quelques tableaux multidimensionnels pendant ma vie et des milliers de tableaux associatifs, mais je n'ai jamais créé de structures de données à partir de rien.

Mais après avoir suivi ce plan d'étude, j'ai de grands espoirs d'être recruté. C'est un long projet. Ça me prendra des mois. Si vous savez déjà beaucoup de choses, ça vous prendra beaucoup moins de temps.
